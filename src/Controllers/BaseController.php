...
abstract class BaseController
{
    ...
    protected EpicDbConnection $db;

    public function __construct(array $overrides = [])
    {
        $this->initializeEpicConfig($overrides);
        $this->db = new EpicDbConnection($overrides);
    }
    ...
    private function authorizeAndStoreToken(string $userId, string $clientId): string
    {
        ...
        $privateKeyPath = $this->epicConfig('private_key_path');
        ...
        cookie()->queue(
            'SessionHash',
            $sessionHash,
            $this->epicConfig('session_cookie_lifetime', 3600),
            '/',
            $this->epicConfig('cookie_domain'),
            true,   // secure
            true,   // httpOnly
            false,  // raw
            'Lax'   // sameSite
